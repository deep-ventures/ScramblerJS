...
if(up==fj)Uj.tj=82;
if(up==fj)Uj.pb=0;
if(up==fj)Uj.ya=19167;


if(up==fj)Uj.bxs="Conviva LivePass version 2.82.0.19167";
...
// Javascript plugin to interface between thePlatform HTML5 Player and Conviva's LivePass API
// Contact:     Alban Nicolas    <anicolas@conviva.com>

// no PDK for some reason
// test $pdk instead? not sure exactly whether testing $pdk would work with older versions or not

if ((typeof tpController !== "undefined") && tpController) {

// in case Javascript context is shared (likely), no need to redefine everything
if (typeof ConvivaPdkPlugin === "undefined" || !ConvivaPdkPlugin) {

// build a class ConvivaPdkPlugin that encapsulates all our variables and methods, by extending the Class object
// could go with $pdk.extend(function,ourclass) instead, not sure exactly whether it would yield anything useful

var parentClass = null;

if (Class && (typeof (Class.extend) == "function")) {
    parentClass = Class;
} else if ($pdk && (typeof ($pdk.extend) == "function")) {
    parentClass = $pdk;
}
...
function PdkStreamerProxy(controller, metadata, plugin) {
    Conviva.ConvivaStreamerProxy.call(this);

    this.controller = controller;
    this.metadata = metadata;
    this.plugin = plugin;
    this.videoElement = $('video', '#' + this.controller.component.player.id)[0];
    this.streamUrl = metadata.streamUrl;
    this.resource = this.metadata.resource;
    this.playingState = Conviva.ConvivaStreamerProxy.STOPPED;
    this.currentBitrateKbps = -1;
    this.capacityKbps = -1;
    this.duration = metadata.duration || -1;
    this.playheadTimeMs = -1;
    this.bufferLengthMs = -1;
    this.inferrer = new Conviva.Inferrer();

    this.setStreamerState = function (newState) {
        // Only update if different
        if (this.playingState !== newState) {
            // Do not overwrite an ERROR state with a STOPPED or UNKNOWN state
            if (this.playingState === Conviva.ConvivaStreamerProxy.ERROR
                && (newState === Conviva.ConvivaStreamerProxy.STOPPED
                    || newState === Conviva.ConvivaStreamerProxy.UNKNOWN)) {
                return;
            }
            // Do the update
    ...
    this.dispatchSwitchingStatus = function() {
        if (this.currentBitrateKbps > 0) {
            this.SetBitrateKbps(this.currentBitrateKbps);
        }
    };

    this.dispatchMetadata = function() {
        if (this.duration >= 0) {
            var meta = {};
            meta[Conviva.ConvivaStreamerProxy.METADATA_DURATION] = this.duration;
            this.SetMetadata(meta);
        }
    };

    this.reportError = function (message) {
        var streamerError = Conviva.StreamerError.makeStreamError(message, Conviva.StreamerError.SEVERITY_FATAL, this.GetStream());
        this.SetError(streamerError);
        this.setStreamerState(Conviva.ConvivaStreamerProxy.ERROR);
    };

    this.Init = function() {
        this.SetResource(this.resource);
        // Cant set the initial player state, no PDK getState()-type methods
        if (this.GetCurrentBitrateKbps() <= 0 && this.metadata.bitrate) {
            this.SetCurrentBitrateKbps(this.metadata.bitrate);
        }
        this.dispatchMetadata();
    };

    // Might be useful later, can be null for now
    function GetStreamerType() {
        return null;
    };

    this.GetIsPHTAccurate = function() {
        return false;
    };

    // This function is called when the monitoring session is terminated
    this.Cleanup = function() {
        // Nothing to do
    };

    // The current playing state: STOPPED, PLAYING, BUFFERING, PAUSED, SEEKING, ERROR, UNKNOWN
    // Note: Some streamers do not have a separate state for seeking.  In such cases, enter the state SEEKING
    // whenever a seek happens and then immediately transition to another state.
    this.GetPlayingState = function() {
        // PDK does not expose any getState() type method, or we might try and refresh state here
        return this.playingState;
    };

    // The bitrate of the currently-playing media, in kilo-bits-per-second (i.e. 1000 bits per second). 
    // When this changes, the callback set by SetSwitchingStatusChangeCallback() should be called with
    // a StreamSwitch targeting the new bitrate, as described in the documentation for that function.
    // -1 if not available.
    this.GetCurrentBitrateKbps = function() {
        if (this.currentBitrateKbps < 0) {
            // I see no way to get the current bitrate unless it's set initially.
            // There should be no bitrate switches in HTMl5
        }
        return this.currentBitrateKbps;
    };

    this.SetCurrentBitrateKbps = function(currentBitrateKbps) {
        if (currentBitrateKbps > 0 && this.currentBitrateKbps != currentBitrateKbps) {
            this.currentBitrateKbps = currentBitrateKbps;
            this.dispatchSwitchingStatus();
        }
    };
    ...
    this.GetPlayheadTimeMs = function() {
        //console.log("###### Requested PHT value: " + this.playheadTimeMs);
        return this.playheadTimeMs;
    };

    this.SetPlayheadTimeMs = function(playheadTimeMs) {
        if (isFinite(playheadTimeMs) && playheadTimeMs >= 0) {
            this.playheadTimeMs = playheadTimeMs;
            //console.log("###### Updated PHT value: " + this.playheadTimeMs);
            this.InferState();
        }
    };
    ...
    this.InferState = function() {
        if (this.inferrer) {
            var newState = this.inferrer.Update(this.playheadTimeMs);
            if (newState) {
                if (newState === Conviva.Inferrer.MOVING_STATE
                    && this.playingState === Conviva.ConvivaStreamerProxy.BUFFERING) {
                    this.setStreamerState(Conviva.ConvivaStreamerProxy.PLAYING);
                    this.plugin.log("Inferred: PLAYING");
                }
                if (newState === Conviva.Inferrer.NOT_MOVING_STATE
                    && this.playingState === Conviva.ConvivaStreamerProxy.PLAYING) {
                    this.setStreamerState(Conviva.ConvivaStreamerProxy.BUFFERING);
                    this.inferredState = Conviva.ConvivaStreamerProxy.BUFFERING;
                    this.plugin.log("Inferred: BUFFERING");
                }
            }
        }
    };
    ...
    this.GetBufferLengthMs = function() {
        // Not exposed by PDK yet
        // Jeremy at thePlatform said it's under consideration
        // getting it directly from html5 video element for now
        if (this.videoElement) {
            var ct = this.videoElement.currentTime;
            var bfr = this.videoElement.buffered;
            if (ct != null && bfr != null) {
                var bft = 0;
                for (var i = 0; i < bfr.length; i++) {
                    if ((bfr.start(i) <= ct) && (bfr.end(i) > ct)) {
                        bft += bfr.end(i) - ct;
                    } else if (bfr.start(i) > ct) {
                        bft += bfr.end(i) - bfr.start(i);
                    }
                }
                this.bufferLengthMs = Math.round(bft * 1000);
                //console.log("###### Updated bufferLengthMs value: " + this.bufferLengthMs);
            }
        }
    ...
    // An estimate of the current capacity, in kilo-bits-per-second (i.e. 1000 bits per 
    // second), of the connection between this client and the current resource.
    this.GetCapacityKbps = function(resource) {
        if (this.resource == resource) { return this.capacityKbps };
        return -1;
    };

    this.SetCapacityKbps = function (capacityKbps) {
        if (isFinite(capacityKbps) && capacityKbps !== this.capacityKbps) {
            this.capacityKbps = capacityKbps;
        }
    };

    this.GetDuration = function () {
        // Duration should always be set via thePlatform metadata, no need to "refresh" it
        return this.duration;
    };

    this.SetDuration = function (duration) {
        if (isFinite(duration) && duration !== this.duration) {
    ...
    this.GetTotalLoadedBytes = function() {
        // Not exposed by the PDK
        return null;
    };
    ...
    this.GetServerAddress = function() {
        // var test = getHostName(this.streamUrl);
        return getHostName(this.streamUrl);
    };
    
    // Return BOOLEAN_TRUE if the streamer is streaming live media, and BOOLEAN_FALSE otherwise.
    // BOOLEAN_UNAVAILABLE if unknown.
    this.GetIsLive = function() {
        if (isFinite(this.duration)) {
            return this.duration <= 0
                ? Conviva.ConvivaStreamerProxy.BOOLEAN_TRUE
                : Conviva.ConvivaStreamerProxy.BOOLEAN_FALSE;
        }
        return Conviva.ConvivaStreamerProxy.BOOLEAN_UNAVAILABLE;
    };

    this.Init();
}

function pluginSetStreamerState(streamer, newState) {
    if (streamer) {
        streamer.setStreamerState(newState);
    }
}

function pluginReportError(streamer, message) {
    if (streamer) {
        streamer.reportError(message);
    }
}
...
ConvivaPdkPlugin = parentClass.extend({
    /** Beginning of Plugin Variables **/
    
    // streamer proxy
    streamer: null,

    // reference to the <video> element to be monitored
    videoElement: null,
    
    // id of the containing element for our videoElement
    // used to find the videoElement
    videoElementAnchor: null,
    
    // stores the current LivePass session
    currentConvivaLivePassSession: null,
    
    // Conviva LivePass credentials
    customerId: null,
    serviceUrl: null,    
    
    convivaPlayerTags: {},

    // raw and parsed versions of Conviva LivePass Parameters passed as arguments
    // convivaParams: null,
    convivaParams: {},
    
    // custom Conviva PDK Event object
    convivaCustomEvent: "OnConvivaCustomEvent",
    
    // scopes used by thePlatform player, passed on when registering event listeners
    scopes: null,
    
    // plugin toggleTraces flag, currently mostly used to toggle Conviva LivePass traces on and off
    toggleTraces: false,

    // plugin toggleRewriteTraces flag, used to toggle traces for url rewriting only
    toggleRewriteTraces: false,
    
    // information about this plugin
    pluginName: "ConvivaThePlatformPlugin",

    // information about the player used for developpement
    pdkName: "thePlatform PDK",
    
    currentPdkVersion: null,
    
    // stream switching support
    // currently unavailable for HTML5 players, but supported nonetheless
    supportStreamSwitching: false,
    
    // copy of the load object used to initialize this instance of the plugin
    lo: null,
    
    // PDK controller we plug into
    controller: null,
    
    // internal ID for our plugin, used to differenciate multiple instances of our plugin on pages with multiple players
    internalId: null,

    /** End of Plugin Variables **/
    
    /** Beginning of Plugin Methods **/
    
    /*** Beginning of Main Initialization Methods ***/
    
    // has to be defined when extending Class? removing it doesn't seem to break anything
    init: function () {
        // doesn't have to do anything regardless though
    },
     
    // plugin initialization function
    // initially takes care of everything from generating an internal id to parsing parameters
    // makes sure the Conviva LivePass Library is loaded and then initialized (through a callback)
    // PDK event listeners are only set up after Conviva LivePass has been initialized
    initialize: function (lo) {
        // randomize a unique id for this plugin
        this.generateInternalId();
        
        this.log("initialize | starting");
        
        // store references to the PDK controller and the load object we'll use to initalize this instance
        this.controller = lo.controller;
        this.lo = lo;
        
        // get some information about the current PDK framework to check if the current version is supported
        this.parseFrameworkInformation();
        
        // get some information about the player anchor and the scopes handled by the PDK controller we plug into
        this.parseControllerInformation();
        
        // retrieve plugin parameters that potentially override default plugin configuration
        // ## make sure all plugin parameters are safe ##
        this.parsePluginParameters();
        
        // retrieve Conviva LivePass parameters we have to pass along 
        // ## LivePass likely takes care of making sure its own arguments are safe, this plugin doesn't make use of them ##
        this.parseConvivaArguments();
        
        // LivePass now bundled into this file, it's always supposed to be loaded when we get here
        this.initLivePass();
        
        // this.controller.registerAdPlugIn(this,lo.priority);    
        // according to the PDK documentation, we're supposed to make a direct call on the controller similar to this one, to let PDK know we're doing fine
        // it doesn't work for now though, we need to pick a specific plugin type (ie AdPlugIn) and we don't really fit the categories
        // if we do choose to pick one of these categories anyway, we'll have to define some more methods specific to each type of plugin
        // for what it's worth, it doesn't seem to matter in practice, everything works fine
        // might only be necessary for plugin that hook themselves in with the intention of modifying something

        // PD-9989: register for url rewriting
        this.controller.registerMetadataUrlPlugIn(this, this.lo.type, this.lo.priority);
        this.log("initialize | registered as MetadataUrlPlugIn");

        // PD-9989
        if ($pdk) {
            this.formats = $pdk.env.Detect.getInstance().getSupportedRuntimes();
            this.log("initialize | supported formats: ", this.formats);
            this.rewritingLogConsole("Conviva Insights | rewriting | supported formats: " + this.formats);
        }
    ...
    rewriteMetadataUrl: function(url, isPreview) {
        this.log("rewriteMetadataUrl | start");

        if (typeof this.manifest === "undefined") {
            this.log("rewriteMetadataUrl | incorrect rewriting parameters, no url rewriting");
            this.rewritingLogConsole("Conviva Insights | rewriting | incorrect rewriting parameters, no url rewriting");
            return false;
        }

        if (!url || isPreview) {
            this.log("rewriteMetadataUrl | preview or empty url, no url rewriting");
            this.rewritingLogConsole("Conviva Insights | rewriting | preview or empty url, no url rewriting");
            return false;
        }

        if (this.pdkShowsHLSCompatibleRuntime()) {
            ...
            this.log("rewriteMetadataUrl | set manifest parameter to: " + this.manifest);
            this.rewritingLogConsole("Conviva Insights | rewriting | set manifest parameter to: " + this.manifest);
        } else {
            if (url.match(/(\?|\&)manifest\=[^\&]*/i)) {
                url = url.replace(/(\?|\&)manifest\=[^\&]*/i, "");
                this.log("rewriteMetadataUrl | stripped manifest parameter");
                this.rewritingLogConsole("Conviva Insights | rewriting | stripped manifest parameter");
            }
            if (this.fallback) {
                if (url.indexOf("?") >= 0) {
                ...
                this.log("rewriteMetadataUrl | using fallback parameter:" + unescape(this.fallback));
                this.rewritingLogConsole("Conviva Insights | rewriting | using fallback parameter: " + unescape(this.fallback));
            } else {
                this.log("rewriteMetadataUrl | no fallback parameter");
                this.rewritingLogConsole("Conviva Insights | rewriting | no fallback parameter");
            }
        }
        
        this.controller.setMetadataUrl(url);
        this.log("rewriteMetadataUrl | set new release url: " + url);
        this.rewritingLogConsole("Conviva Insights | rewriting | set new release url: " + url);

        return true;
    },
    
    pdkShowsHLSCompatibleRuntime: function()
    {
        if (tpIsAndroid() && !this.androidHLS) {
            this.log("isManifestCompatible | Android platform but androidHLS parameter is false");
            this.rewritingLogConsole("Conviva Insights | rewriting | Android platform but androidHLS is false");
            return false;
        }

        for (var i=0; i<this.formats.length; i++) {
            if (this.formats[i].match(/html5\:m3u/i)) {
                this.log("isManifestCompatible | " + this.formats[i] + " is manifest compatible");
                this.rewritingLogConsole("Conviva Insights | rewriting | " + this.formats[i] + " format is manifest compatible");
                return true;
            }
        }

        this.log("isManifestCompatible | no manifest compatible formats available");
        this.rewritingLogConsole("Conviva Insights | rewriting | no manifest compatible formats available");
        return false;
    },

    /** PD-9989 END **/

    // checks if LivePass has loaded correctly
    // if so it initializes LivePass, and then sets up PDK event listeners
    initLivePass: function (sfn) {
        if (typeof Conviva !== "undefined" && Conviva) {
            // initialize if necessary
            if (!Conviva.LivePass.ready) { // check if LivePass is already initialized

                // setup LivePass traces based on plugin toggleTraces status
                Conviva.LivePass.toggleTraces(this.toggleTraces);

                var that = this;
                Conviva.LivePass.init(this.serviceUrl, this.customerId, function (n) { that.livePassNotifier(n); });

                this.log("initLivePass | Conviva LivePass initialized");
            } else {
                this.log("initLivePass | Conviva LivePass already initialized");
            }

            // now set up event listeners via PDK controller
            this.setupEventListeners();
        } else {
            this.error("initLivePass | Conviva LivePass script failed to load");
            // not much else to do, we're in place but useless since LivePass isn't working
            // guess we could setTimeout this method again in case there is a simple delay, most likely it will fail again though
            
        }
    },

    // sets up PDK event listeners via the PDK controller we plugged into
    // refer to individual event listeners for more details
    setupEventListeners: function () {
        var me = this;
        
        this.controller.addEventListener(PdkEvent.OnReleaseSelected, function (e) { me.onReleaseSelected(e); }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnReleaseStart, function (e) { me.onReleaseStart(e); }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaLoadStart, function (e) { me.onMediaLoadStart(e) }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaComplete, function (e) { me.onMediaComplete(e) }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaBuffer, function (e) { me.onMediaBuffer(e) }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaPlay, function (e) { me.onMediaPlay(e) }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaPause, function (e) { me.onMediaPause(e) }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaUnpause, function (e) { me.onMediaUnpause(e) }, this.scopes);
        this.controller.addEventListener(PdkEvent.OnMediaPlaying, function (e) { me.onMediaPlaying(e) }, this.scopes);

        this.controller.addEventListener(PdkEvent.OnMediaError, function (e) { me.onMediaError(e) }, this.scopes);
    
        //this.controller.addEventListener(PdkEvent.OnStreamSwitched, function (e) { me.onStreamSwitched(e) }, this.scopes);    // not yet supported in HTML5
        
        // internal testing
        /*
        if (this.toggleTraces) {
            this.controller.addEventListener(PdkEvent.OnMediaSeek, function (e) { me.onMediaSeek(e) }, this.scopes);
        }
        */

        // other potentially useful listeners, seem unnecessary for the time being
        // this.controller.addEventListener("OnMediaStart", function (e) { me.onMediaStart(e) }, this.scopes);
        // this.controller.addEventListener("OnMediaEnd", function (e) { me.onMediaEnd(e) }, this.scopes);
        
        // also catch custom events in global scopes, so that it works in pages with a single pdk player even if the customer fails to specify the correct scope
        // an event only triggers a specific handler once, even if the handler listens to multiple scopes the event was sent to
        // this means our handler listening on multiple scopes won't result in us reacting to the same event multiple times
        // however, multiple conviva plugins coexisting on the same page will define different handlers, and in this case, the customer failing to specify the correct scope for each custom event will indeed duplicate it
        // commenting the next two lines will make interferences impossible, at the cost of forcing the customer to always specify the correct player scopes, even on pages with a single pdk player (events published to the default scopes will never trigger our handler in that case)
        var tmp = this.scopes.slice();
        tmp.push("default");
        tmp.push("javascript");
        
        this.controller.addEventListener(this.convivaCustomEvent, function (e) { me.onConvivaCustomEvent(e) }, tmp);
        
        this.log("setupEventListeners | done");
    },
    
    /*** End of Main Initialization Methods ***/
    
    /*** Beginning of Secondary Initialization Methods ***/
    
    // grabs version information from the global PDK object
    // might be able to get it from this.controller somehow?
    parseFrameworkInformation: function () {
        if ($pdk && $pdk.version) {
            v = $pdk.version;
            this.currentPdkVersion = v.major + "." + v.minor + "." + v.revision + "." + v.build;
        }
    },
    
    // grabs useful information from the PDK controller we plugged into
    parseControllerInformation: function () {
        // get the dom id for the parent HTML element that will contain the <video> player
        this.videoElementAnchor = this.controller.id + ".content";
        this.log("parseControllerInformation | videoElementAnchor: ", this.videoElementAnchor);
        //this.updateVideoElement();
        
        // get the event scopes
        this.scopes = this.controller.scopes;
        this.log("parseControllerInformation | scopes: ", this.scopes.join(','));
        
        // this.log("parseControllerInformation | done");
    },
    
    parseParameter: function (dest,key,bool) {
        if (typeof bool == "undefined" || bool == null) {
            bool = false;
        }
        if (this.lo.vars.hasOwnProperty(key)) {
            var v = this.lo.vars[key];    // decode here if we ever bring param values URLEncoding back
            if (bool) {
                if (v === "true") { v = true; }
                if (v === "false") { v = false; }
            }
    ...
    parseParametersArray: function (dest,prefix) {
        for (var key in this.lo.vars) {
            mts = key.match("^"+prefix+"(.*)");        // turns "prefixXXXXX" into ["prefix","XXXXX"]
            if (mts && mts[1] != "") {
                dest[mts[1]] = this.lo.vars[key];    // decode here if we ever bring param values URLEncoding back
            }
        }
    },
    
    // parses initialization parameters for the plugin
    // used to override default plugin behaviour without modifying the javascript code
    // would potentially be dangerous if we had anything to protect against users modifying this file before using it anyway
    // i don't think we do so, let's just call this handy
    parsePluginParameters: function () {
        this.parseParameter(this,"toggleTraces",true);

        this.parseParameter(this,"toggleRewriteTraces",true);
        
        this.parseParameter(this,"supportStreamSwitching",true);

        // PD-9989: grab plugin parameters for url rewriting
        this.parseParameter(this,"manifest",false);
        if (this.manifest === "true" || this.manifest === "false") {
            this.manifest = "m3u"
        }
        this.parseParameter(this,"fallback",false);
        if (!this.fallback) {
            this.fallback = null;
        }
        this.parseParameter(this,"androidHLS",true);
        if (!this.androidHLS) {
            this.androidHLS = false;
        }

        this.rewritingLogConsole("Conviva Insights | rewriting | manifest: " + this.manifest + " / fallback: " + this.fallback + " / androidHLS: " + this.androidHLS);
        
        this.log("parsePluginParameters | done");
    },
    
    // takes care of getting all the Conviva-related arguments from the load object, parsing them and storing them in this instance of the plugin
    parseConvivaArguments: function () {
        this.log("parseConvivaArguments | starting");
        
        this.parseConvivaCredentials();
        this.parseConvivaParameters();
        this.parseConvivaTags();
        
        this.log("parseConvivaArguments | done");
    },
    
    // parses Conviva credentials arguments
    // simply shuts down if credentials are missing
    parseConvivaCredentials: function () {
        this.parseParameter(this,"customerId");
        
        this.parseParameter(this,"serviceUrl");

        if (!this.customerId) {
            // no customer id, exiting
            this.error("parseConvivaCredentials | 'customerId' is not present in the plugin configuration!");
            return;
        }
        
        if (!this.serviceUrl) {
            // no service url, exiting
            this.error("parseConvivaCredentials | 'serviceUrl' is not present in the plugin configuration");
            return;
        }
        
        // found credentials, continue on
        this.log("parseConvivaCredentials | done");
    },
    
    // parses Conviva LivePass parameters in plugin arguments
    // used to override default ConvivaContentInfo values without modyfing the plugin javascript code
    parseConvivaParameters: function () {
        this.parseParameter(this.convivaParams,"playerName");
        
        this.parseParameter(this.convivaParams,"cdnName");

        this.parseParameter(this.convivaParams,"resource");

        this.parseParameter(this.convivaParams,"deviceId");
        
        this.parseParameter(this.convivaParams,"deviceType");
        
        this.parseParameter(this.convivaParams,"viewerId");
        
        this.parseParameter(this.convivaParams,"metadataKeys");
        
        this.log("parseConvivaParameters | done");
    },
    
    // parses Conviva Tags
    // used to pass along customized media tags directly via the host page
    parseConvivaTags: function () {
        this.parseParametersArray(this.convivaPlayerTags,"playerTag\.");    // prefix is used in a regex, escape it correctly
        
        this.log("parseConvivaTags | done");
    },
    
    /*** End of Secondary Initialization Methods ***/
    
    /** End of Initialization Methods **/
    
    // /** Beginning of Custom PDK Event Dispatchers Methods **/
    
    // // dispatches a custom Conviva PDK Event with corresponding payload to the correct scopes
    // dispatchConvivaEvent: function (payload) {
        // console.log('dispatching OnConvivaCustomEvent in scope ' + this.scopes);
        // this.controller.dispatchEvent(this.convivaCustomEvent, payload, this.scopes);
    // },
    
    // /** End of Custom PDK Event Dispatchers Methods **/

    /** Beginning of PDK Event Listeners Methods **/
    
    /*** Beginning of Primary PDK Event Listeners Methods ***/
    
    // event listener for 'OnMediaLoadStart' PDK event
    // indicates a new media is being loaded into the player
    // calls for a new Conviva LivePass session
    onMediaLoadStart: function (e) {
        // this.log("onMediaLoadStart | starting");
    
        this.createConvivaLivePassSession(e);

        this.log("onMediaLoadStart | done");
    },

    onReleaseSelected: function (e) {
        // this.log("onReleaseSelected | starting");

        if (e && e.data && e.data.releaseUrl) {
            this.fetchPlayerName(e.data.releaseUrl);
        }

        this.log("onReleaseSelected | done");
    },

    onReleaseStart: function (e) {
        // this.log("onReleaseStart | starting");

        if (e && e.data && e.data.releaseURL) {
            this.fetchPlayerName(e.data.releaseURL);
        }

        this.log("onReleaseStart | done");
    },

    // event listener for 'OnMediaBuffer' PDK event
    // indicates media is buffering
    onMediaBuffer: function (e) {
        this.log("onMediaBuffer | starting");

        if (!this.currentConvivaLivePassSession) { return; }
    
        pluginSetStreamerState(this.streamer, Conviva.ConvivaStreamerProxy.BUFFERING);
        if (e && e.data && e.data.clip && e.data.clip.hasOwnProperty("currentMediaTime")) {
          this.streamer.SetPlayheadTimeMs(e.data.clip.currentMediaTime);
        }
        //this.streamer.capacityKbps = e.data.connectionBitrate;

        //this.log("onMediaBuffer | done");
    },

    // event listener for 'OnMediaPlay' PDK event
    // indicates play is resuming after buffering
    onMediaPlay: function (e) {
        this.log("onMediaPlay | starting");

        if (!this.currentConvivaLivePassSession) { return; }
    
        pluginSetStreamerState(this.streamer, Conviva.ConvivaStreamerProxy.PLAYING);
        if (e && e.data && e.data.clip && e.data.clip.hasOwnProperty("currentMediaTime"))  {
          this.streamer.SetPlayheadTimeMs(e.data.clip.currentMediaTime);
        }
        //this.streamer.capacityKbps = e.data.connectionBitrate;

        //this.log("onMediaPlay | done");
    },

    // event listener for 'OnMediaUnpause' PDK event
    // indicates play is resuming after pause
    onMediaUnpause: function (e) {
        this.log("onMediaUnpause | starting");
        ...
        //this.log("onMediaUnpause | done");
    },

    // event listener for 'OnMediaPause' PDK event
    // indicates the player was paused, either by user or by internal logic
    // do we want to skip internal pauses?
    onMediaPause: function (e) {
        this.log("onMediaPause | starting");

        if (!this.currentConvivaLivePassSession) { return; }
    
        pluginSetStreamerState(this.streamer, Conviva.ConvivaStreamerProxy.PAUSED);
        if (e && e.data && e.data.clip && e.data.clip.hasOwnProperty("currentMediaTime")) {
          this.streamer.SetPlayheadTimeMs(e.data.clip.currentMediaTime);
        }
        //this.streamer.capacityKbps = e.data.clip.connectionBitrate;
        // check e.data.userInitiated (boolean)

        //this.log("onMediaPause | done");
    },

    // event listener for 'OnMediaPlaying' PDK event
    // indicates the media is actually playing
    // fired about 3 times a second
    onMediaPlaying: function (e) {
        //this.log("onMediaPlaying | starting");

        if (!this.currentConvivaLivePassSession) { return; }
    
        // we still receive this event while PDK fails to detect we're buffering
        // we don't want to deduce playing state from receiving this event if we already
        // inferred buffering state
        if (this.streamer && this.streamer.inferredState !== Conviva.ConvivaStreamerProxy.BUFFERING) {
            pluginSetStreamerState(this.streamer, Conviva.ConvivaStreamerProxy.PLAYING);
        }

        if (e && e.data && e.data.hasOwnProperty("mediaTime")) {
          this.streamer.SetPlayheadTimeMs(e.data.mediaTime);
        }
        //this.streamer.duration = e.data.duration;
        // e.data.isLive ??

        //this.log("onMediaPlaying | done");
    },
    
    // event listener for 'OnMediaComplete' PDK event
    // indicates the current media has ended
    // PDK documentation indicates it only triggers when the media ran its natural course to the end
    // current tests show it actually triggers even when the user switches media, making it the cleanest known PDK event indicating it's time to end a Conviva LivePass session
    onMediaComplete: function (e) {
        // this.log("onMediaComplete | starting");
        
        this.endConvivaLivePassSession();
        
        this.log("onMediaComplete | done");
    },
    
    // event listener for 'OnMediaError' PDK event
    // indicates some sort of failure in the current playback
    onMediaError: function (e) {
        // this.log("onMediaError | starting");

        this.convivaLivePassReportError("streaming");
        
        // e.data.message "The media cannot be played" can be thrown during buffering, ending session there isn't good
        // this.endConvivaLivePassSession();
        
        this.log("onMediaError | done");
    },
    
    // event listener for 'OnConvivaCustom' custom Conviva PDK event
    // no idea what we'd use that for, but here it is
    onConvivaCustomEvent: function (e) {
        this.log("onConvivaCustomEvent | event: ", e);
        
        /*
        // for testing
        if (typeof e.data === "string" && this.toggleTraces) {
            e.data = this._debugIFramePayloadDecode(e.data);
        }
        */
        
        var eventName = e.data.name;
        var eventAttributes = e.data.attributes;

        if (eventName && (eventName != "")) {
            if (Conviva && Conviva.LivePass && Conviva.LivePass.metrics) {
                Conviva.LivePass.metrics.sendEvent(eventName, eventAttributes);
            }
        } else {
            this.log("onConvivaCustomEvent | invalid eventName");
        }
        
        this.log("onConvivaCustomEvent | done");
    },
    
    // event listener for 'OnMediaComplete' PDK event
    // indicates the current media has ended
    // PDK documentation indicates it triggers on multiple occasions, including when a chapter of the current media ends (read: potentially when the user seeks ahead)
    // there doesn't seem to be any reliable way to distinguish between the reasons why this event was triggered
    // thus currently 'OnMediaComplete' is a more viable option
    onMediaEnd: function (e) {
        this.endConvivaLivePassSession();
        
        this.log("onMediaEnd | done");
    },
    
    // event listener for 'onMediaSeek' PDK event
    onMediaSeek: function (e) {
        this.log("onMediaStart | done");
    },

    // event listener for 'onStreamSwitched' PDK event
    // indicates the current media has switched stream
    // calls for a bitrate information update on current Conviva LivePass session
    // not yet be supported in HTML5
    onStreamSwitched: function (e) {
        if (!this.currentConvivaLivePassSession) return;
        
        var bitrate = -1;
        try {
            bitrate = Math.round(e.data.newFileInfo.bitrate / 1000);
        } catch (err) { }
        
        this.streamer.SetCurrentBitrateKbps(bitrate);
        
        this.log("onStreamSwitched | done");
    },
    
    /*** End of Primary PDK Event Listeners Methods ***/
    
    /*** Beginning of Secondary PDK Event Listeners Methods ***/
    
    createConvivaLivePassSession: function (e) {
        // this.log("createConvivaLivePassSession | starting");

        var clip = e.data;

        // PD-10124: ignore audio content
        if (clip.baseClip && clip.baseClip.type
            && clip.baseClip.type.match(/audio\//i)) {
            this.log("createConvivaLivePassSession | audio track, ignoring");
            return;
        }
        
        // don't bother with bogus Clip objects or ads
        // ## primary track stuff? ##
        // Note sure if there is distinguish between primary and non-primary tracks.
        if (clip.baseClip && !clip.baseClip.isAd) { 
            if (this.currentConvivaLivePassSession) { 
                this.endConvivaLivePassSession();
                
                // not really normal behaviour though, drop a warning
                this.warn("createConvivaLivePassSession | forced currentConvivaLivePassSession to end, to start a new session");
            }
        
            var convivaMetadata = this.buildConvivaContentInfoFromClip(clip);

            // make sure we got the right <video> player
            // monitoring will fail if the <video> player was changed by the PDK and we don't get the latest
            this.updateVideoElement();

            this.streamer = new PdkStreamerProxy(this.controller, convivaMetadata, this);
            
            // start a new Conviva LivePass session
            this.currentConvivaLivePassSession = Conviva.LivePass.createSession(this.streamer, convivaMetadata);
            
            this.log("createConvivaLivePassSession | created LivePass session");
            
            if (clip.baseClip.mediaLength) {    // clip.baseClip.releaseLength) {
                // contentLength information was provided by the PDK, passing it on to Conviva LivePass manually
                this.currentConvivaLivePassSession.setContentLength(Math.round(clip.baseClip.mediaLength / 1000));    // clip.baseClip.releaseLength / 1000));
                this.log("createConvivaLivePassSession | setContentLength on LivePass session");
            }
        } else {
            if (!clip.baseClip) {
                this.log("createConvivaLivePassSession | bogus track: no baseClip");
            } else if (clip.baseClip.isAd) {
                this.log("createConvivaLivePassSession | ad track, ignoring");
            }
        }
        
        // this.log("createConvivaLivePassSession | done");
    },
    
    convivaLivePassReportError: function (type) {
        if (!this.currentConvivaLivePassSession) { return; }
        
        switch(type) {
            case "streaming":
                this.currentConvivaLivePassSession.reportError(Conviva.ConvivaLightSession.ERROR_STREAMING_FAILURE);
                this.warn("convivaLivePassReportError | passed along streaming error");
                break;
        }
    },
    
    getBitrateFromClip: function (clip) {
        var br = null;
        
        if (clip.trueBitrate) {
            pi = parseInt(clip.trueBitrate,10);
            if (!isNaN(pi) && (pi > 0)) {
                br = pi;
            }
        }
        
        if (!br && clip.baseClip.bitrate) {
            pi = parseInt(clip.baseClip.bitrate,10);
            if (!isNaN(pi) && (pi > 0)) {
                br = pi;
            }
        }
        ...
        return Math.floor(br / 1000);
    },
    
    buildConvivaContentInfoFromClip: function (clip) {
        // PD-11957: always use Playlist.title over Clip.title if it exists
        // As of PDK 5.3.12, the new Playlist.title property supposedly always contains the title we are looking for.
        var title;
        if (clip && clip.baseClip && clip.baseClip.playlistRef && clip.baseClip.playlistRef.title) {
            title = clip.baseClip.playlistRef.title;
        } else { // Legacy, PDK < 5.3.12
            title = clip.title;
        }
        
        var assetName = "[" + (clip.baseClip.contentID ? clip.baseClip.contentID : "noID") + "] " + (title ? title : "noTitle");

        //console.log(clip);

        var convivaMetadata = Conviva.ConvivaContentInfo.createInfoForLightSession(assetName);
        
        convivaMetadata.cdnName = this.convivaParams.cdnName ? this.convivaParams.cdnName : Conviva.ConvivaContentInfo.CDN_NAME_OTHER; // required    

        convivaMetadata.resource = this.convivaParams.resource;

        convivaMetadata.streamUrl = clip.URL; // required
        // this.videoElement.src is sometimes different from clip.URL (ie includes a cache busting query string or something), not sure which one we prefer to log, probably the base clip.URL.
        // if using this.videoElement.src, make sure to call this.updateVideoElement beforehand

        // does not exist in ConvivaContentInfo, still useful to pass it to the streamer proxy
        convivaMetadata.duration = clip.mediaLength;

        // both of these have been 0 for non-live media at one point or another during the tests, careful :|
        convivaMetadata.isLive = (clip.mediaLength <= 0);    // required
        // convivaMetadata.isLive = clip.baseClip.releaseLength <= 0 ? "true" : "false";

        // if we need it updated for an already existing session, use
        // this.currentConvivaLivePassSession.setCurrentBitrate(newBitrateKbps);
        convivaMetadata.bitrate = this.getBitrateFromClip(clip);
            
        convivaMetadata.pluginName = this.pluginName;
        
        convivaMetadata.pluginVersion = Conviva.LivePass.version;
            
        //convivaMetadata.playerName = this.convivaParams.playerName ? this.convivaParams.playerName : this.pdkName + ' ' + this.currentPdkVersion;
        if (this.convivaParams.playerName) {
            convivaMetadata.playerName = this.convivaParams.playerName;
        }
        
        convivaMetadata.frameworkName = Conviva.ConvivaContentInfo.FRAMEWORK_NAME_THE_PLATFORM;
        convivaMetadata.frameworkVersion = this.currentPdkVersion;
        
        convivaMetadata.ovppName = Conviva.ConvivaContentInfo.OVPP_NAME_THE_PLATFORM;
        
        if (this.convivaParams.viewerId) {
            convivaMetadata.viewerId = this.convivaParams.viewerId;
        }                

        //convivaMetadata.deviceType = this.convivaParams.deviceType ? this.convivaParams.deviceType : Conviva.ConvivaContentInfo.DEVICE_TYPE_PC;
        if (this.convivaParams.deviceType) {
            convivaMetadata.deviceType = this.convivaParams.deviceType;
        }
        
        if (this.convivaParams.deviceId) {
            convivaMetadata.deviceId = this.convivaParams.deviceId;
        }
        
        var tmpTags = jQuery.extend({}, this.convivaPlayerTags);

        // PD-9457: handle c3.playerName and ext.thePlatform.player tags and default values.
        this.setPlayerNameTags(convivaMetadata, tmpTags, clip);
        
        if (clip.baseClip.contentCustomData != null) {
            var logKeys = [];
            if (typeof(clip.baseClip.contentCustomData.keys) === "function") {
                logKeys = clip.baseClip.contentCustomData.keys();
            } else {
                // Mozilla backward compatibility snippet
                for (var key in clip.baseClip.contentCustomData) {
                    if (Object.prototype.hasOwnProperty.call(clip.baseClip.contentCustomData, key)) {
                        logKeys.push(key);
                    }
                }
            }
            this.log("buildConvivaContentInfoFromClip | customKeys: " + logKeys.join(','));
            
            // add metadataKeys to tags
            if (this.convivaParams.hasOwnProperty("metadataKeys") && (clip.baseClip.contentCustomData != null)) {
                ekeys = this.convivaParams["metadataKeys"].split(',');
                // console.log(clip.baseClip.contentCustomData);
                for (var key in ekeys) {
                    var v = clip.baseClip.contentCustomData[ekeys[key]];
                    // console.log(key + ": " + clip.baseClip.contentCustomData[key]);
                    if (v) {
                        //this.convivaPlayerTags[ekeys[key]] = v;
                        tmpTags[ekeys[key]] = v;
                    }
                }
            }
        ...
        // add MPX identifiers to tags, if available
        if (clip.baseClip.hasOwnProperty("id")) {
            tmpTags["tp.clip.id"] = clip.baseClip.id;
        }
        if (clip.baseClip.hasOwnProperty("contentID")) {
            tmpTags["tp.clip.contentID"] = clip.baseClip.contentID;
        }
        if (clip.baseClip.hasOwnProperty("guid")) {
            tmpTags["tp.clip.guid"] = clip.baseClip.guid;
        }
        
        // never seen clip.baseClip.rawData != null
        // i've seen trackingData and copyright directly in baseClip though
        if (clip.baseClip.rawData != null) {
            this.setAidTag(clip.baseClip.rawData, tmpTags);
        } else {
            this.setAidTag(clip.baseClip, tmpTags);
        }

        // PD-10138: add categories tags
        try {
            if (clip && clip.baseClip && clip.baseClip.categories) {
                var categories = clip.baseClip.categories;
                for (var index in categories) {
                    var category = categories[index];
                    if (category.name) {
                        var tag = ['ext.thePlatform.categories', '[', index, ']'].join('');
                        tmpTags[tag] = category.name;
                    }
                }
            }
        } catch (e) {}

        // pass along Conviva LivePass tags
        convivaMetadata.tags = tmpTags;
        
        return convivaMetadata;
    },

    setPlayerNameTags: function (convivaMetadata, tags, clip) {
        // get official PDK player name
        var url = null;
        if (clip && clip.baseClip && clip.baseClip.playlistRef && clip.baseClip.playlistRef.releaseURL) {
            url = clip.baseClip.playlistRef.releaseURL;
        }
        var pdkPlayerName = this.fetchPlayerName(url);
        if (pdkPlayerName) {
            // use it to set ext.thePlatform tag
            if (!tags["ext.thePlatform.player"]) {
                tags["ext.thePlatform.player"] = pdkPlayerName;
            }
            // use it as default value for playerName tag
            if (!convivaMetadata.playerName) {
                convivaMetadata.playerName = pdkPlayerName;
            }
        }
    },

    fetchPlayerName: function (url) {
        if (this._playerName) { return this._playerName; }
        if (url) {
            var value = this.getParameterValueFromUrl(url, "player", true);
            if (value) {
    ...
    getParameterValueFromUrl: function (url, requestedKey, unEscape) {
        if (url && requestedKey) {
            var queryString = url.split('#')[0].split('?')[1];
            if (queryString) {
                var parameters = queryString.split('&');
                for (var kvPairIndex in parameters) {
                    var kvPair = parameters[kvPairIndex];
                    var equalSplit = kvPair.split('=');
                    var key = equalSplit[0];
                    var value = equalSplit[1];
                    if (key == requestedKey && value) {
                        if (unEscape) { value = decodeURIComponent(value); }
                        return value;
                    }
                }
            }
    ...
    getPlayerTag: function (bcRawData) {
        if (!bcRawData) { return null; }

        var trackingData = bcRawData.trackingData || bcRawData.copyright;
        
        if (trackingData) {
            //this.log("getPlayerTag | trackingData : " + trackingData);
        
            var playerObject = (/pl=(\d+)/).exec(trackingData);
            
            if (playerObject && playerObject[1]) {
                var tag = playerObject[1];
                this.log("getPlayerTag | extracted player: " + tag);
                return tag;
            }
        }
        return null;
    ...
    setAidTag: function (bcRawData, tags) {
        if (!bcRawData)
            return;
        ...
        if (trackingData) {
            //this.log("setAidTag | trackingData : " + trackingData);
        
            var aidObject = (/aid=(\d+)/).exec(trackingData);
            
            if (aidObject && aidObject[1]) {
                tags['ext.thePlatform.aid'] = aidObject[1];
                this.log("setAidTag | extracted aid: " + tags['ext.thePlatform.aid']);
            }
        }
    },
    
    // ends the current Conviva LivePass session
    // if there is no current LivePass and this function was called without first making sure there was, most likely some previous PDK event cleared the LivePass session when we weren't expecting it to
    endConvivaLivePassSession: function () {
        if (this.currentConvivaLivePassSession) {
            // end of this LivePass session
            Conviva.LivePass.cleanupMonitoringSession(this.streamer);
            try { this.currentConvivaLivePassSession.cleanup(); } catch (e) {} // never works for some reason
            this.currentConvivaLivePassSession = null;
            
            this.log("endConvivaLivePassSession | done");
        } else {
            // not blocking, but chances are PDK events aren't handled properly somehow
            
            this.log("endConvivaLivePassSession | no currentConvivaLivePassSession to end, how did we end up here?");
        }
    },
    
    /*** End of Secondary PDK Event Listeners Methods ***/
    
    /** End of PDK Event Listeners Methods **/
    
    /** Beginning of Misc Methods **/
    
    // callback for Conviva LivePass, init method mostly
    // also triggered by Insights sometimes if we send out invalid data or use it too frequently
    livePassNotifier: function (cn) {
        if (cn.code == 0) {
            // Conviva LivePass confirms successfully initialisation
            this.log("livePassNotifier | Conviva LivePass initialized successfully" );
        } else {
            if (cn.code == 30) {    // ConvivaNotification.ERROR_METRICS_QUOTA_EXCEEDED
                this.warn("livePassNotifier | Conviva LivePass Insights ERROR_METRICS_QUOTA_EXCEEDED.\n " +
                             "\tCode: " + cn.code + ";\n " +
                             "\tMessage: " + cn.message);
            } else {
                if (Conviva.LivePass.ready) { // check if LivePass is already initialized
                    // not really a problem as far as I can tell
                    this.log("livePassNotifier | Conviva LivePass post-initialization feedback.\n " +
                             "\tCode: " + cn.code + ";\n " +
                             "\tMessage: " + cn.message); 
                } else {
                    // now that's a big problem
                    this.error("livePassNotifier | Conviva LivePass failed to initialize!\n " + "t\Conviva metrics will not be captured! " + "\tCode: " + cn.code + "; " + "\tMessage: " + cn.message);
                }
            }
        }
        this.log("livePassNotifier | convivaNotification [code: " + cn.code + "; objectId : " + cn.objectId + "; message: " + cn.message + "]");
    },
    
    /** End of Misc Methods **/
    
    /** Beginning of Internal Methods **/
    
    // uses videoElementAnchor and basic DOM methods to pinpoint the <video> player Conviva LivePass will be monitoring, then stores it
    updateVideoElement : function () {
        // fetch HTML element containing the <video> player
        // based on information provided by the PDK controller
        var el = document.getElementById(this.videoElementAnchor);
        if (el) {
            // found the element, trying to fetch <video> element
            var fc = el.firstChild;
            if (fc && (fc.tagName == "VIDEO")) {
                // found <video> element
                // storing videoElement and moving on
                this.videoElement = fc;
                return;
            }
        }
        // maybe thePlatform changed its player's HTML structure
        this.error("updateVideoElement | couldn't find <video> player in element ", el);
    },
    
    // generate a pseudo-random internal id for this instance of the plugin
    // used to differentiate between instances when the host page contains multiple players running this plugin
    generateInternalId: function () {
        this.internalId = Math.round(1000000 * Math.random());
    },
    
    // internal logging function, normal log level
    // generally no impact on plugin behavior
    log: function () {
        if (this.toggleTraces) {
    ...
    // internal logging function, warn log level
    // you can expect plugin to not behave as you wanted it to as long as one of these shows up in the console
    warn: function () {
        if (this.toggleTraces) {
    ...
    // internal logging function, error log level
    // generally fatal error, will cause the plugin to either shutdown if it hit during normal flow, or pretty much fail at everything if it occured in a delayed callback
    error: function () {
        if (this.toggleTraces) {
    ...
    rewritingLogConsole: function (msg) {
        if (this.toggleRewriteTraces) {
            if (typeof console === 'undefined') { return; }
            console.log(msg);
        }
    },

    // prefixes messages with pertinent information about the plugin
    // then uses the console if present to display the messages for different log levels
    logConsole: function (level, args) {
        if (typeof console === 'undefined') { return; }
        
        var f, anchor, scope;
        
        if (this.videoElementAnchor) {
            anchor = this.videoElementAnchor;
        } else {
            // videoElementAnchor information wasn't retrieved from controller yet, mark as To Be Determined
            anchor = "TBD";
        }
        ...
        if (this.scopes) {
            ...
            // scopes information wasn't retrieved from controller yet, mark as To Be Determined
            scope = "TBD";
        }
        
        var details = " ( #" + anchor + " / " + scope + " ) ";
        var prefix = '#' + this.internalId + details + this.pluginName + ' ' + Conviva.LivePass.version + ': ';
        var args = [].splice.call(args, 0);
    ...
    // shouldn't happen, pdk comes packaged with Class.extend and (later versions) $pdk.extend
    alert('Error: extend unavailable for some reason.');
}
...
var latestConvivaPdkPlugin = new ConvivaPdkPlugin();

// check in with PDK
tpController.plugInLoaded(latestConvivaPdkPlugin);

// store all instances of our plugin somewhere, useful for debugging sometimes
if ((typeof convivaPdkPlugins == "undefined") || !convivaPdkPlugins) { convivaPdkPlugins = []; }
convivaPdkPlugins.push(latestConvivaPdkPlugin);

} else {

alert('Error: Conviva PDK plugin loaded outside of PDK context somehow.');
